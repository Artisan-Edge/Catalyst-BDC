...
export async function updateAnalyticModel(
    requestor: DatasphereRequestor,
    space: string,
    csn: CsnFile,
    objectName: string,
): AsyncResult<string> {
    const objectType = DATASPHERE_OBJECT_TYPES['analytic-model'];

    const [payload, extractErr] = extractObject(csn, objectType.csnKey, objectName);
    if (extractErr) return err(extractErr);

    debug(`Updating analytic model "${objectName}"...`);
    ...
    return checkResponse(response, reqErr, `Update analytic model "${objectName}"`);
}
